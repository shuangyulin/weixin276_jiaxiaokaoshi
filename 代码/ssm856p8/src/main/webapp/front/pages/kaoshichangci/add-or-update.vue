	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`kaoshichangci`, this.ruleForm.id);
		...
		methods: {
			...
			async onSubmitTap() {
//跨表计算判断
				if((!this.ruleForm.kaoshichangci)){
					this.$utils.msg(`考试场次不能为空`);
					return
				}
				//更新跨表属性
			       var crossuserid;
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`kaoshichangci`, this.ruleForm);
						}else{
							await this.$api.add(`kaoshichangci`, this.ruleForm);
						}
