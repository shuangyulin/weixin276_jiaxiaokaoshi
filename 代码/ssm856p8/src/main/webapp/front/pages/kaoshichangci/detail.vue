...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','kaoshichangci');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','kaoshichangci');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('kaoshichangci', this.id);
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discusskaoshichangci/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('kaoshichangci', this.detail);
				this.$utils.msg('审核成功');
